...
float rayon = 200.0, decalageY = 20;
...
void draw() {
  ...
  rotateX(radians(((height/2)-mouseY)/2.5));
 ...
 // 2. Méthode optimisée, avec le mode QUAD_STRIP ou TRIANGLE_STRIP
  beginShape(QUAD_STRIP);
  for (int j=0; j<4; j++) {
    for (int i=0; i<=reso; i++) {
      if (i == reso) {
        vertex(pointsCercle[0][0], -decalageY+(j*(2*decalageY)), pointsCercle[0][1]);
        vertex(pointsCercle[0][0], decalageY+(j*(2*decalageY)), pointsCercle[0][1]);
      } 
      else {
        vertex(pointsCercle[i][0], -decalageY+(j*(2*decalageY)), pointsCercle[i][1]);
        vertex(pointsCercle[i][0], decalageY+(j*(2*decalageY)), pointsCercle[i][1]);
      }
    }
  }
